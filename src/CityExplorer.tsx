...
import {MapIcon,PersonIcon,RouteIcon,SearchIcon} from './icons';
...
export default function CityExplorer({onLegacy,onPilotAssets,initialPilot,onStationExperience}:{onLegacy:()=>void;onPilotAssets:(asset?:PilotAssetId)=>void;initialPilot?:PilotAssetId;onStationExperience:()=>void}){
 ...
 // 2,294 bus stops are on the map and in the routing, but they are not somewhere anyone browses to,
 // and counting them made this list claim 4,256 places while the same screen said 1,975. They stay
 // findable by name: someone who types their stop still gets it, and can set it as either endpoint.
 const filtered=useMemo(()=>{const q=normalize(query);return (data?.places??[]).filter(p=>(q||!p.id.startsWith('bus-'))&&(district==='전체'||p.district===district)&&(category==='all'||p.category===category||(category==='elevator'&&p.facilities.includes('elevator'))||(category==='toilet'&&p.facilities.includes('toilet')))&&(!q||normalize(p.name+' '+p.district+' '+p.address).includes(q))).sort((a,b)=>{const score=(p:CityPlace)=>(p.verified?100:0)+(/^(한빛탑|대전역|대청댐|대전오월드|뿌리공원|유림공원|우암사적공원|동춘당공원|엑스포과학공원)$/.test(p.name)?50:0)+(['attraction','culture','park'].includes(p.category)?5:0);return score(b)-score(a)||a.name.localeCompare(b.name,'ko');});},[data,query,district,category]);
...
function choose(p:CityPlace){setPreview(false);setFollow(false);setPanelOpen(true);setSelected(p);if(picking){if(picking==='from')setFrom(p);else setTo(p);setPicking(null);setQuery('');}view.current?.fly(placeCoordinate(p),pilotAssetForPlace(p.id)?16.8:17,window.innerWidth<=740?0:DETAIL_INSET);setMobileMode(picking?'route':'place');setSheetLevel(picking?'half':'peek');mobileSearch.current?.blur();}
 ...
 function demoFinished(played:boolean){
  if(played||!showcase)return;
  setShowcase(false);setPanelOpen(true);setFollow(false);
 }
 ...
 return <div className={'daejeon-app mobile-'+mobileMode+' sheet-'+sheetLevel+(panelOpen?'':' panel-collapsed')+(picking?' is-picking':'')+(showcase?' is-showcase':'')}>
  ...
  <a className="skip-link" href="#dj-map-region">지도로 건너뛰기</a>
  <header className="dj-header"><h1 className="dj-brand-heading"><a className="dj-brand" href="/"><img src="/brand/mark-128.png" alt=""/><strong>갈수있슈<small>나에게 맞는 대전 여행길</small></strong></a></h1><div className="dj-location"><i/>대전광역시 <span>5개 구 전체</span></div><nav aria-label="지도 화면"><button className="active" aria-current="page">대전 지도</button><button onClick={()=>onPilotAssets()}>3D 에셋 살펴보기</button></nav><button className="dj-intro-link" onClick={()=>{history.pushState(null,'','/intro');setIntro(true);}}>▶ 소개 영상</button></header>
  <main className="dj-layout">
   <nav className="desktop-rail" aria-label="지도 도구"><button aria-current={panelMode==='search'?'page':undefined} onClick={()=>{setPanelOpen(true);setPanelMode('search');setSelected(null);setQuery('');setMobileMode('browse');search.current?.focus();}}><SearchIcon />장소 찾기</button><button aria-current={panelMode==='route'?'page':undefined} onClick={()=>{setPanelOpen(true);setPanelMode('route');setSelected(null);}}><RouteIcon />길찾기</button><button onClick={()=>setSurvey(true)}><PersonIcon />마이</button></nav>
   <aside id="journey-sidebar" className={'dj-panel '+(sheet?'expanded':'')} aria-label="장소와 이동 계획">
    ...
    <footer className="dj-data-footer">{coverage&&<div className="dj-coverage"><strong>대전 {coverage.places.toLocaleString()}곳 가운데 휠체어 접근성이 적힌 곳은 {coverage.documented.toLocaleString()}곳이에요.</strong><figure className="dj-coverage-meter"><svg viewBox="0 0 600 18" role="img" aria-label={`대전에 기록된 장소 ${coverage.places.toLocaleString()}곳 가운데 휠체어 접근성이 표기된 곳은 ${coverage.documented.toLocaleString()}곳이에요.`}><rect x="0" y="0" width="600" height="18" rx="6" fill="#dfd6cf"/><rect x="0" y="0" width={Math.max(5,600*coverage.documented/coverage.places)} height="18" rx="6" fill="#c05f42"/></svg></figure><p>나머지 {(coverage.places-coverage.documented).toLocaleString()}곳은 갈 수 있는지조차 알 수 없어요. 저희가 채우려는 빈칸이에요.</p><details><summary>자세한 숫자</summary><ul><li><b>{coverage.toiletsDocumented}</b> 공개 화장실 {coverage.toilets}곳 중 휠체어 화장실로 확인된 곳</li><li><b>{coverage.elevators}</b> 엘리베이터로 기록된 지점</li>{tour&&<><li><b>{tour.counts.barrierFree}</b> 관광공사 무장애 안내가 있는 곳 (관광지 {tour.counts.total}곳 중)</li><li><b>{tour.counts.barrierFreeHearing}</b> 그중 청각 안내가 등록된 곳</li></>}</ul><small>OpenStreetMap 대전 스냅샷 {coverage.snapshot.slice(0,10)} · 한국관광공사 TourAPI 기준</small></details></div>}<strong>등록된 정보만 표시해요</strong><p>건물 외곽선은 OSM·Overture 자료를 함께 사용해요. 높이가 없는 건물은 추정 높이로 표현하며, 빠진 건물이 있을 수 있어요. 다리는 등록된 위치를 따르지만 폭·난간은 일부 재구성이에요. 모형은 접근성 판단 자료가 아니에요.</p><p><a href="/data/building-data-report.json" target="_blank" rel="noreferrer">건물 데이터 출처·보완 내역 ↗</a> · <a href="/data/building-footprints.geojson.gz" download>외곽선 데이터</a></p><button onClick={onLegacy}>기존 한밭수목원 여행 화면 ↗</button></footer>
   </div></aside>
  ...
  <MobilitySurvey open={survey} onOpenChange={setSurvey} value={prefs} onSave={savePrefs} companion={companion}/>
