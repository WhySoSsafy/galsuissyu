...
function choose(p:CityPlace){setPreview(false);setFollow(false);setPanelOpen(true);setSelected(p);if(picking){if(picking==='from')setFrom(p);else setTo(p);setPicking(null);setQuery('');}view.current?.fly(placeCoordinate(p),pilotAssetForPlace(p.id)?16.8:17,window.innerWidth<=740?0:DETAIL_INSET);setMobileMode(picking?'route':'place');setSheetLevel(picking?'half':'peek');mobileSearch.current?.blur();}
 ...
 return <div className={'daejeon-app mobile-'+mobileMode+' sheet-'+sheetLevel+(panelOpen?'':' panel-collapsed')+(picking?' is-picking':'')}>
  ...
  <main className="dj-layout">
   <nav className="desktop-rail" aria-label="지도 도구"><button aria-current={panelMode==='search'?'page':undefined} onClick={()=>{setPanelOpen(true);setPanelMode('search');setSelected(null);setQuery('');setMobileMode('browse');search.current?.focus();}}><span aria-hidden="true">⌕</span>장소 찾기</button><button aria-current={panelMode==='route'?'page':undefined} onClick={()=>{setPanelOpen(true);setPanelMode('route');setSelected(null);}}><span aria-hidden="true">⇄</span>길찾기</button><button onClick={()=>setSurvey(true)}><span aria-hidden="true">👤</span>마이</button></nav>
  ...
  <nav className="dj-mobile-nav" aria-label="주요 기능"><button aria-current={mobileMode==='browse'?'page':undefined} onClick={()=>{setMobileMode('browse');setPanelMode('search');setSelected(null);setPicking(null);setSheet(false);mobileSearch.current?.blur();}}><span>⌖</span>지도</button><button aria-current={mobileMode==='search'?'page':undefined} onClick={()=>{setSelected(null);setPanelMode('search');setMobileMode('search');setSheet(true);mobileSearch.current?.focus();}}><span>⌕</span>검색</button><button aria-current={mobileMode==='route'?'page':undefined} onClick={()=>{setPanelMode('route');setMobileMode('route');setSelected(null);setPicking(null);setSheetLevel(route?'full':'half');mobileSearch.current?.blur();}}><span>⇄</span>길찾기</button><button onClick={()=>setSurvey(true)}><span aria-hidden="true">👤</span>마이</button></nav>
