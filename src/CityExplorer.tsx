...
function choose(p:CityPlace){setPreview(false);setFollow(false);setPanelOpen(true);setSelected(p);if(picking){if(picking==='from')setFrom(p);else setTo(p);setPicking(null);setQuery('');}view.current?.fly(placeCoordinate(p),pilotAssetForPlace(p.id)?16.8:17,window.innerWidth<=740?0:DETAIL_INSET);setMobileMode(picking?'route':'place');setSheetLevel(picking?'half':'peek');mobileSearch.current?.blur();}
 ...
 function runDemo(){
  ...
  setTravelMode('transit');setPanelOpen(true);setMobileMode('route');setSheetLevel('peek');
  setAutoRun(n=>n+1);
 }
 ...
 return <div className={'daejeon-app mobile-'+mobileMode+' sheet-'+sheetLevel+(panelOpen?'':' panel-collapsed')}>
  ...
  <nav className="dj-mobile-nav" aria-label="주요 기능"><button aria-current={mobileMode==='browse'?'page':undefined} onClick={()=>{setMobileMode('browse');setSelected(null);setPicking(null);setSheet(false);mobileSearch.current?.blur();}}><span>⌖</span>지도</button><button aria-current={mobileMode==='search'?'page':undefined} onClick={()=>{setSelected(null);setMobileMode('search');setSheet(true);mobileSearch.current?.focus();}}><span>⌕</span>검색</button><button aria-current={mobileMode==='route'?'page':undefined} onClick={()=>{setMobileMode('route');setPicking(null);setSheetLevel(route?'full':'half');mobileSearch.current?.blur();}}><span>⇄</span>길찾기</button><button onClick={()=>setSurvey(true)}><span>☷</span>이동 조건</button></nav>
